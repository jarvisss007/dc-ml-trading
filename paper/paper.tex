...
\paragraph{Reproducibility.} \texttt{python run\_walkforward.py} reproduces
Table~\ref{tab:main}; \texttt{python extra\_analysis.py},
\texttt{context\_regime.py}, and \texttt{plots.py} reproduce the scaling-law,
feature-importance, cost, model-comparison, regime, and figure results. Data are
the three series bundled in the \texttt{arch} package; no vendor access is
required.
